-- Add jump_post_id to topic list RPCs:
-- - oldest unread post id for viewer when available
-- - otherwise latest post id
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(unread_first.post_id, lp.last_post_id) AS jump_post_id,
  ...
  LEFT JOIN LATERAL (
    SELECT p_unread.id AS post_id
    FROM posts p_unread
    WHERE p_unread.topic_id = t.id
      AND p_unread.deleted_at IS NULL
      AND p_unread.author_id <> auth.uid()
      AND (mv.last_viewed_at IS NULL OR p_unread.created_at > mv.last_viewed_at)
    ORDER BY p_unread.created_at ASC, p_unread.id ASC
    LIMIT 1
  ) unread_first ON true
  LEFT JOIN LATERAL (
    SELECT
      tag_row.name,

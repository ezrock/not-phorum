-- Edit topic "header" fields through first post edit flow.
-- Allows topic author or admin to edit:
-- - topic title
-- - first post content/image
-- - primary topic tag (single tag, fallback off-topic)
...
BEGIN
  ...
  SELECT
    ...
    tg.name,
    tg.slug,
    COALESCE(NULLIF(btrim(tg.icon), ''), '🏷️')
  FROM topics t

...
-- 1) do not overwrite new-forum profile registration dates
-- 2) keep unmatched legacy users without a synthetic created date
...
BEGIN
  ...
  WITH normalized_legacy AS (
    SELECT
      ...
      NULL::timestamptz AS legacy_created_at,
  ...
  SELECT
    ...
    CASE
      WHEN m.profile_id IS NULL THEN NULL
      ELSE m.current_created_at

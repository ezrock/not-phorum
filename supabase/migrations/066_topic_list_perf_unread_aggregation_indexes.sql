-- Topic list performance hardening:
-- 1) add indexes for latest-post and unread scans
-- 2) aggregate unread stats per page topic set (avoid per-row LATERAL scans)

CREATE INDEX IF NOT EXISTS idx_posts_topic_created_visible
  ON posts (topic_id, created_at DESC, id DESC)
  WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_posts_topic_author_created_visible
  ON posts (topic_id, author_id, created_at DESC, id DESC)
  WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_topic_view_events_viewer_topic_last_viewed
  ON topic_view_events (viewer_key, topic_id, last_viewed_at);

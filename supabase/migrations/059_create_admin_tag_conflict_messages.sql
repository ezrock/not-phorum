-- Improve admin tag creation conflict handling with explicit messages.
...
BEGIN
  ...
  IF conflict_tag_name IS NOT NULL THEN
    RAISE EXCEPTION 'Tag name already exists: %', conflict_tag_name;
  END IF;
  ...
  EXCEPTION
    WHEN unique_violation THEN
      -- Race-safe error message if another insert won between pre-check and INSERT.
      SELECT t.name
      INTO conflict_tag_name
      FROM tags t
      WHERE t.slug = normalized_slug
